...
async function chackUserWork(userData, datawork) {
  try {
    ...
    if (Status.length == 0) {
      ...
      await tbUserWork.updateOne({ chat_id: userData.id, workType: datawork },
        {
          $set: {
            status: true
          }
...
async function selectDataWork(TypeWorker) {
  try {
    return await tbUserWork.find()
      .and([{ status: true }, { workType: TypeWorker }])
      .select('firstName name payment_amount payment_type address telNumber -_id');
  } catch (ex) {
