...
async function all_work() {
    return await tbUser.find().countDocuments();
}
// faqat hamma ro'yxatni chiqarish...
async function old_work() {
    return await tbUser.find({ status: false }).countDocuments();
}
// faqat xozirda mavjud ro'yxatni chiqarish...
async function active_work() {
    return await tbUser.find({ status: true }).countDocuments();
}
// endi workerlarni chiqarish uchun...
async function all_worker() {
    return await tbUserWork.find().countDocuments();
}
// faqat hamma ro'yxatni chiqarish...
async function old_worker() {
    return await tbUserWork.find({ status: false }).countDocuments();
}
async function active_worker() {
    return await tbUserWork.find({ status: true }).countDocuments();
}
// start bosgan odamlar sonini chiqarish...
async function all_users() {
    return await tbBot.find().countDocuments();
}
async function users_chat_ids() {
    return await tbBot.find().select("chat_id -_id");
}
module.exports = {
    all_work,
    old_work,
    ...
    all_users,
    // barcha chat idilarni qaytaradi
    users_chat_ids,
};
